# py01_gpt_clone.py
# GUI를 위한 모듈
from tkinter import * # tkinter 모듈에 있는 모든 클래스, 함수, 변수 등을 사용 / Tk, scrolledtext, messagebox
...
# 6. 제미나이 API용 구성
genai.configure(api_key="AIzaSyANCLGU18CAYv0TMU_Pu1MDwlD8AtU2h68") # 신청한 API키 
model = genai.GenerativeModel('gemini-1.5-flash')# 사전훈련된 AI모델
...
# 12. 아이콘 변경
# ./image/ 경로 삭제
# root.iconbitmap('./image/chatbot.ico') # pyinstaller로 설치할 땐 해당폴더에 복사
root.iconbitmap('chatbot.ico') # pyinstaller 때는 실행파일 폴더에 icon 복사
...
# 2. UI화면 구성
inputFrame = Frame(root, width=730, height=30, bg='#DFDFDF', padx=10)
inputFrame.pack(side=BOTTOM, fill=BOTH)

# 3. inputFrame에 들어갈 Entry와 Button 구성, wrap=WORD 글자단위로 워드 랩핑
textMessage = Text(inputFrame, width=87, height=1, wrap=WORD, font=myFont)
...
buttonSend.pack(side=RIGHT, padx=8, pady=5)
# 글을 적을 위젯 (Entry)

# 5. API호출 결과메시지 출력될 스크롤기능 텍스트위젯
textResult = ScrolledText(root, wrap=WORD, bg='#333333', fg='white', font=myFont) # bg='black'
textResult.pack(fill=BOTH, expand=True)

# 10. 스크롤 텍스트에 나올 메시지 디자인
textResult.tag_configure('user', font=boldFont, foreground='yellow') # 태그가 user인 것 폰트와 fg 변경
