...
class Block:
    ...
    def move(self): # 볼 무브
        ...
        # 볼이 움직이는 y축 값을 계속 계산하려면 y는 dir 값을 라디언으로 변환 후 사인처리
...
def main():
    ...
    score = 0
    BLOCK = []
    BALL = Block((200,200,0), Rect(375, 650, 20, 20), 10) # 공 생성, 속도 10
    ...
    while True:
        ...
        Surface.fill((33, 33, 33)) # Surface.fill((0,0,0))
            ...
            if event.type == QUIT: 
                ...
                if event.key == K_LEFT:
                    if PADDLE.rect.centerx < 50:
                        ...
                        PADDLE.rect.centerx -= 10 # 패달은 왼쪽 오른쪽만 이동 가능
        ...
        if is_game_start == False:
            ...
            LenBlock = len(BLOCK) # 54개로 시작, 공에 충돌해서 갯수 줄어듬
            # Collision Dectection(충돌 탐지)
